...
public class StringQuery {
        ...
        public final String QUERY_GET_ALL = "SELECT * FROM %s;";

        /*
         * Запросы с таблицей выполненных работ
         */

        public final String QUERY_INSERT_ACCOUNT = "INSERT INTO data_work(\"group\", count, name, price, summa, unit_meas, vat) "
                        +
                        "VALUES (?, ?, ?, ?, ?, ?, ?);";

        public final String QUERY_DELETE_ACCOUNT = "DELETE FROM data_work " +
                        "WHERE \"group\" = ? AND count = ? AND name = ? AND price = ? AND summa = ? AND unit_meas = ? AND vat = ?;";

        public final String QUERY_UPDATE_ACCOUNT = "UPDATE data_work " +
